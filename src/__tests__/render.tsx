...
describe('Render fields with default values', () => {
    ...
    test('Default object', () => {
        ...
        return renderDefault(schema).then(v =>
            expect(v).not.toBe(schema.value)
        );
    });
    test('Default array', () => {
...
describe('Handle the unknown', () => {
    ...
    test("Doesn't send update if value didn't change", () => {
        ...
        for (let i = 0; i < 10; i++) {
            render(
                <Container schema={schema} value={i} onChange={onChange} />,
                { container }
            );
        }
    ...
    test("Doesn't send update for same input", () => {
        ...
        for (let i = 0; i < 10; i++) {
            render(
                <Container schema={schema} value={value} onChange={onChange} />,
                { container }
            );
        }
        expect(onChange).not.toBeCalled();
